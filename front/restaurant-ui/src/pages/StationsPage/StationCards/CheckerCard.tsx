...
export default function CheckerCard({
  ...
  const [productScope, setProductScope] = useState<"food" | "drinks" | "both">("both");
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setProductScope(settings?.productScope ?? "both");
      } catch (e) {
  ...
  async function handleSave() {
    try {
      ...
      setProductScope(saved?.productScope ?? "both");
    } catch (e) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 text-sm text-gray-700">
      ...
      <div className="text-xs text-gray-500">
        Choose the revenue center and product scope this checker owns so routed
        orders land on the right queue during service.
      </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_auto] md:items-end">
        ...
        <label className="space-y-2">
          ...
          <select
            ...
            disabled={loading || saving}
            onChange={(e) =>
              setProductScope(e.target.value as "food" | "drinks" | "both")
            }
          >
      ...
      <div className="mt-3 rounded-xl border border-dashed border-gray-200 bg-gray-50 px-3 py-3 text-xs text-gray-600">
        ...
        <div className="mt-1 text-gray-500">
          `Both` handles everything. `Food` and `Drinks` let one revenue center
          split across two checker stations.
        </div>
      </div>
